...
static char enter_buffer[NUM_TERMINALS][BUFFER_SIZE];
...
static volatile unsigned char enter_flag[NUM_TERMINALS];
  ...
  /* Receive the character pressed */
	...
	else // handle key pressed down (flag keys and normal key recognition)
	{
		...
		if ((alt_flag == 1) && (c == F1_MAKE || c == F2_MAKE || c == F3_MAKE))
		{
			...
			current_active_terminal = c - F1_MAKE;
			change_video_mem(old_num, current_active_terminal);
			
		}
		else if (key == '\b') // backspace handling code
		{
				input_buffer[current_active_terminal][curr_buff_pos[current_active_terminal]] = 0;
		...
		else if ((key == '\n') || (key == '\r')) // handle return
		{
			...
			{
				enter_buffer[current_active_terminal][i] = input_buffer[current_active_terminal][i];
...
extern int32_t terminal_open()
{
	...
	{
		input_buffer[current_active_terminal][i] = 0;
		enter_buffer[current_active_terminal][i] = 0;
	}
...
extern int32_t terminal_read(fd_t* file_desc, uint8_t* buf, uint32_t nbytes)
{
	...
	for (i = 0; i < cnt; i++)
	{
		...
		buf[i] = enter_buffer[term_num][i];
		if (enter_buffer[term_num][i] == '\0')
		{
			break;
		}
	}
